...
class AdminController extends Controller
{
    ...
    {
        if (!Auth::user() || !in_array(Auth::user()->usertype, [1])) abort(404);
    ...
    public function storeAdmin(Request $request)
    {
       ...
       DB::beginTransaction();
        try {
            ...
            DB::rollBack();
            Log::error('#### AdminController -> storeAdmin() #### ' . $e->getMessage());
            Session::flash('alert-error', __('message.something_went_wrong'));
    ...
    public function editAdmin($uuid)
    {
        ...
        try {
            ...
            Log::error('####### AdminController -> editAdmin() #######  ' . $e->getMessage());
            Session::flash('alert-error', __('message.something_went_wrong'));
            return redirect()->back();
        }
    }
    ...
    public function updateAdmin(Request $request, $uuid)
    {
        ...
        try {
            ...
            $adminDetails->usertype       = $request->usertype;
            $adminDetails->save();
            ...
            DB::commit();
            Session::flash('alert-success', __('message.admin_updated_successfully'));
            return redirect()->back();
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function updateImage(Request $request,$uuid)
    {
        ...
        try {
            ...
            Log::error('####### AdminController -> updateImage() #######  ' . $e->getMessage());
            Session::flash('alert-error', __('message.something_went_wrong'));
            return redirect()->back();
        }
    }
    ...
    public function destroyAdmin($uuid)
    {
      ...
      if (!auth_permission_check('Delete Admin')) return redirect()->back();
        ...
        try {
            ...
            DB::rollBack();
            Log::error('####### AdminController -> destroyAdmin() #######  ' . $e->getMessage());
            Session::flash('alert-error', __('message.something_went_wrong'));
            return redirect()->back();
    ...
    {  
        ...
        if (!auth_permission_check('View Admin Details')) return Datatables::of([])->make(true);

        #main query
        $query  = User::with('roles:id,name','media')
        ->where('usertype', '1');
