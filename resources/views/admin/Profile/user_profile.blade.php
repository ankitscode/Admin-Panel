                    ...
                    <form method="POST" action="{{ route('admin.updateProfile') }}" enctype="multipart/form-data">
                        ...
                        <div class="row gx-3 mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input class="form-control @error('address') is-invalid @enderror" id="address"
                                    name="address" type="tel" placeholder="{{ __('main.Enter your address') }}"
                        ...
                        <div class="row gx-3 mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-control @error('city_type') is-invalid @enderror" name="city_type">
                                    ...
                                    @foreach ($cityTypes as $key => $cityType)
                                        <option value="{{ $key }}"
                                            {{ isset(Auth::user()->city_type) && Auth::user()->city_type == $key ? 'selected' : '' }}>
                                            {{ $cityType }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select class="form-control @error('country_type') is-invalid @enderror"
                                    name="country_type">
                                    <option value="">{{ __('main.select') }}</option>
                                    @php
                                    ...
                                    @foreach ($countryTypes as $key => $countryType)
                                        <option value="{{ $key }}"
                                            {{ isset(Auth::user()->country_type) && Auth::user()->country_type == $key ? 'selected' : '' }}>
                                            {{ $countryType }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="card-body">
                    <div class="card-body text-center">
                        <img class="rounded-circle mb-2 avater-ext"
                            src="{{ !empty(Auth::user()->media->name) ? '/storage/images/profile/' . Auth::user()->media->name : asset('assets/images/users/user-dummy-img.jpg') }}"
                        ...
                        <button class="btn btn-soft-primary" data-bs-toggle="modal"
                            data-bs-target="#adminProfileImageUpdateModal">{{ __('main.update_profile_image') }}</button>
                    </div>
                </div>
            </div>
        </div>
