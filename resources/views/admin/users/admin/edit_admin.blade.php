        ...
        <div class="card shadow mb-4">
            <form method="POST" action="{{ route('admin.updateAdmin', ['uuid' => $adminDetails->uuid]) }}"
                ...
                <div class="card-header py-3 d-flex justify-content-between align-items-center" style="background: none">
                    <h6 class="m-0 font-weight-bold text-primary">{{ __('main.admin_details') }}</h6>
                    <div class="form-check form-switch">
                        <input class="form-check-input mx-1" name="is_active" type="checkbox" role="switch" id="is_active" value="{{ old($adminDetails->is_active) }}" @if (old('is_active', $adminDetails->is_active)) checked @endif>
                ...
                <div class="card-body">
                    ...
                    <div class="row gx-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('gender_type') is-invalid @enderror" name="gender_type"
                                data-choices>
                                @php
                                ...
                                @foreach ($genders as $key => $gender)
                                    <option value="{{ $key }}"
                                        {{ isset($adminDetails->gender_type) && $adminDetails->gender_type == $key ? 'selected' : '' }}>
                                        {{ $gender }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select mb-3 @error('role_ids') is-invalid @enderror" name="role_ids"
                                id="role_ids" data-choices>
                                @php
                                ...
                                @if (count($roles))
                                    ...
                                    @foreach ($roles as $role)
                                        <option value="{{ $role->name }}"
                                            {{ isset($assignedRole) && in_array($role->name, $assignedRole) ? 'selected' : '' }}>
                                            {{ $role->name }}</option>
                                    @endforeach
                                @endif
                            </select>
